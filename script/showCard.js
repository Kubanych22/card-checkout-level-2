...
export const showCard = () => {
  const paymentTitle = el('div', {className: 'payment-title'});
  const h1 = el('h1', {}, 'Payment Information');
  const container = el('div', {className: 'container'});
  
  setChildren(paymentTitle, h1);
  
  const creditcard = document.createElement('div');
  creditcard.classList.add('creditcard');
  const front = document.createElement('div');
  front.classList.add('front');
  const ccsingle = document.createElement('div');
  ccsingle.id = 'ccsingle';
  
  let svg = document.createElement('svg');
  svg.innerHTML = `<svg version="1.1" id="cardfront" xmlns="http://www.w3.org/2000/svg"
          ...
          <g id="Front">
            ...
            <text transform="matrix(1 0 0 1 54.1064 428.1723)" id="svgname"
              class="st2 st5 st6">ИВАН ИВАНОВ</text>
  ...
  front.append(ccsingle, svg);
  
  const back = el('div', {className: 'back'});
  
  svg = document.createElement('svg');
  svg.innerHTML = `<svg version="1.1" id="cardback" xmlns="http://www.w3.org/2000/svg"
          xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
          viewBox="0 0 750 471" style="enable-background:new 0 0 750 471;"
          xml:space="preserve">
          <g id="Front">
          ...
          C0,17.9,17.9,0,40,0z" />
            ...
            <text transform="matrix(1 0 0 1 59.5073 228.6099)" id="svgnameback"
              class="st12 st13">Иван Иванов</text>
          </g>
        </svg>`
  
  back.append(svg);
  
  creditcard.append(front, back);
  container.append(creditcard);
  ...
  const formContainer = el('div', {className:'form-container'},
    ...
    el('div', {className:'field-container'},
      ...
      el('input', {id:'cardnumber', pattern:'[0-9]*', type:'text', inputmode:
        'numeric'}),
      el('svg', {id:'ccicon', className:'ccicon', width:'750', height:'471',
        viewBox:'0 0 750 471', version:'1.1', xmlns:'http://www.w3.org/2000/svg',
        xmlnsxlink:'http://www.w3.org/1999/xlink'})),
    el('div', {className:'field-container'},
      el('label', {for:'expirationdate'}, 'Expiration (mm/yy)'),
      el('input', {id:'expirationdate', pattern:'[0-9]*', type:'text', inputmode:
          'numeric'})
      ),
    el('div', {className:'field-container'},
      el('label', {for:'securitycode'}, 'Security Code'),
      el('input', {id:'securitycode', pattern:'[0-9]*', type:'text', inputmode:
          'numeric'})
      )
  )
